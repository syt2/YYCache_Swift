...
class YYKVStorageSwift {
    ...
    init?(path: URL, type: YYKVStorageType) {
        ...
        do {
            ...
            debugPrint("YYKVStorage init error: \(error.localizedDescription))")
            return nil
        }
        if !dbOpen() || !dbInitialize() {
            ...
            guard dbOpen(), dbInitialize() else {
                dbClose()
                debugPrint("YYKVStorage init error: fail to open sqlite db.")
                return nil
            }
        }
    ...
    deinit {
        let taskId = Self.YYSharedApplication?.beginBackgroundTask(expirationHandler: nil)
        dbClose()
        if let taskId = taskId, taskId != .invalid {
            Self.YYSharedApplication?.endBackgroundTask(taskId)
        }
    }
    
    func saveItem(_ item: YYKVStorgeItem) -> Bool {
    ...
    func getItem(key: String) -> YYKVStorgeItem? {
        guard !key.isEmpty,
    ...
    func getItemInfo(key: String) -> YYKVStorgeItem? {
    ...
    func getItems(keys: [String]) -> [YYKVStorgeItem]? {
    ...
    func getItemInfos(keys: [String]) -> [YYKVStorgeItem]? {
        guard !keys.isEmpty else { return nil }
...
private extension YYKVStorageSwift {
    ...
    func dbBindJoin(keys: [String], stmt: OpaquePointer?, fromIndex: Int) {
        keys.enumerated().forEach {
            sqlite3_bind_text(stmt, Int32($0.offset + fromIndex), $0.element, -1, nil)
        }
    }
    ...
    func dbGetItem(stmt: OpaquePointer!, excludeInlineData: Bool) -> YYKVStorgeItem? {
        
        guard let keyPointer = sqlite3_column_text(stmt, 0), keyPointer.pointee != 0 else { return nil }
        var item = YYKVStorgeItem(key: String(cString: keyPointer))
    ...
    func dbGetItem(key: String, excludeInlineData: Bool) -> YYKVStorgeItem? {
    ...
    func dbGetItems(keys: [String], excludeInlineData: Bool) -> [YYKVStorgeItem]? {
    ...
    func dbGetItemSizeInfoOrderByTimeAsc(limit: Int) -> [YYKVStorgeItem]? {
